...
## コーディングに限らず、常にYAGNIを厳守しろ

## ログディレクトリ以外のファイル編集禁止

- ログディレクトリ以外のファイルwrite操作は、完全に禁止
- CLAUDE.mdやMakefileなど全てwriteを禁止する
...
- **同一話題継続の判定**: 同じタイトル（意味内容）の場合は既存ファイルに追記
- **時刻による判定禁止**: 時刻での同一セッション判別は禁止
- **意味内容による判定**: 話題の意味的関連性のみでセッション継続を判断
